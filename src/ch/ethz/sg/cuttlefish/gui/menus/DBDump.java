...
import ch.ethz.sg.cuttlefish.gui.tasks.DumpToDBWorker;
	...
	private void initAttrComponents() {
        ...
        attrDump.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
				...
				(new DumpToDBWorker(networkPanel, (CxfNetwork)networkPanel.getNetwork(), nodeField.getText(), linksField.getText(), conn, nodeAttrs, linkAttrs, replaceTables.isSelected()) ).execute();							
            	selectAttrs.setVisible(false);
            }
        });
